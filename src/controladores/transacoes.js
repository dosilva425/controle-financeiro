...
const { validadorCamposPreenchidosRegistroTransacao, validadorCategoria, validadorTipo, validadorTransacaoDoUsuarioLogado } = require("../utilitarios/funcoesValidadoras-utilitario");
...
const cadastrarTransacao = async (req, res) => {
  ...
  try {
    if (!validadorCamposPreenchidosRegistroTransacao(req, res)) {
      return res.status(400).json({ mensagem: "Todos os campos obrigatórios devem ser informados." });
    }
    if (!(await validadorCategoria(req, res))) {
...
const listarTransacoes = async (req, res) => {
  ...
  try {
    const transacoes = await pool.query("select * from transacoes where usuario_id = $1", [idUsuarioLogado(req, res)]);
    ...
    if (filtros) {
      ...
      for (let categoria = 0; categoria < filtros.length; categoria++) {
        const categoriaFiltroNome = await pool.query(
          "select categorias.descricao from categorias where categorias.descricao ilike $1",
          [filtros[categoria]]
        );
        if (categoriaFiltroNome.rowCount < 1) {
          return res.status(400).json({ mensagem: "A categoria de filtragem informada não existe." });
        }

        const transacoesDetalhadasPorCategoria = await pool.query(
          `select t.id, t.tipo, t.descricao, t.valor, t.data, t.usuario_id, t.categoria_id, c.descricao as "categoria_nome"
            from transacoes t inner join categorias c on t.categoria_id = c.id where t.usuario_id = $1 and c.descricao ilike $2
          `, [idUsuarioLogado(req, res), filtros[categoria]]
        );
        listaTransacoesPorCategoria.push(transacoesDetalhadasPorCategoria.rows);
      }

      return res.status(200).json(listaTransacoesPorCategoria);
    }
  } catch (error) {
    return res.status(500).json({ mensagem: "Erro interno do servidor" });
  }
...
const detalharTransacao = async (req, res) => {
  ...
  try {
    if (!(await validadorTransacaoDoUsuarioLogado(req, res))) {
...
const excluirTransacao = async (req, res) => {
  ...
  try {
    ...
    const excluirTransacao = pool.query("delete from transacoes where id = $1", [id]);
